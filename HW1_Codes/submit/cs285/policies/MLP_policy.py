"""
Defines a pytorch policy as the agent's actor

Functions to edit:
    2. forward
    3. update
"""

import abc
import itertools
from typing import Any
from torch import nn
from torch.nn import functional as F
from torch import optim

import numpy as np
import torch
from torch import distributions

from cs285.infrastructure import pytorch_util as ptu
from cs285.policies.base_policy import BasePolicy


def build_mlp(
        input_size: int,
        output_size: int,
        n_layers: int,
        size: int
) -> nn.Module:
    """
        Builds a feedforward neural network

        arguments:
            n_layers: number of hidden layers
            size: dimension of each hidden layer
            activation: activation of each hidden layer

            input_size: size of the input layer
            output_size: size of the output layer
            output_activation: activation of the output layer

        returns:
            MLP (nn.Module)
    """
    layers = []
    in_size = input_size
    for _ in range(n_layers):
        layers.append(nn.Linear(in_size, size))
        layers.append(nn.Tanh())
        in_size = size
    layers.append(nn.Linear(in_size, output_size))

    mlp = nn.Sequential(*layers)
    return mlp


class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    """
    Defines an MLP for supervised learning which maps observations to continuous
    actions.

    Attributes
    ----------
    mean_net: nn.Sequential
        A neural network that outputs the mean for continuous actions
    logstd: nn.Parameter
        A separate parameter to learn the standard deviation of actions

    Methods
    -------
    forward:
        Runs a differentiable forwards pass through the network
    update:
        Trains the policy with a supervised learning objective
    """
    def __init__(self,
                 ac_dim,
                 ob_dim,
                 n_layers,
                 size,
                 learning_rate=1e-4,
                 training=True,
                 nn_baseline=False,
                 **kwargs
                 ):
        super().__init__(**kwargs)

        # init vars
        self.ac_dim = ac_dim
        self.ob_dim = ob_dim
        self.n_layers = n_layers
        self.size = size
        self.learning_rate = learning_rate
        self.training = training
        self.nn_baseline = nn_baseline

        self.mean_net = build_mlp(
            input_size=self.ob_dim,
            output_size=self.ac_dim,
            n_layers=self.n_layers, size=self.size,
        )
        self.mean_net.to(ptu.device)
        self.logstd = nn.Parameter(

            torch.zeros(self.ac_dim, dtype=torch.float32, device=ptu.device)
        )
        self.logstd.to(ptu.device)
        self.optimizer = optim.Adam(
            itertools.chain([self.logstd], self.mean_net.parameters()),
            self.learning_rate
        )

    def save(self, filepath):
        """
        :param filepath: path to save MLP
        """
        torch.save(self.state_dict(), filepath)

    def forward(self, observation: torch.FloatTensor) -> Any:
        #if isinstance(observation, np.ndarray):
        #    observations = torch.tensor(observations).float().to(ptu.device)
            
        ##if isinstance(actions, np.ndarray):
        #    actions = torch.tensor(actions).float().to(ptu.device)
        """
        Defines the forward pass of the network

        :param observation: observation(s) to query the policy
        :return:
            action: sampled action(s) from the policy
        """
        # TODO: implement the forward pass of the network.
        # You can return anything you want, but you should be able to differentiate
            # mean_net 來得到連續動作的均值，並利用 logstd 得到標準差。最後，我們可以利用這兩者的值來從正態分布中採樣動作
        # through it. For example, you can return a torch.FloatTensor. You can also
        # return more flexible objects, such as a 
        # `torch.distributions.Distribution` object. It's up to you!
        
        ###
        
        action_mean = self.mean_net(torch.from_numpy(observation).float())
        action_std = torch.exp(self.logstd)
        distribution = distributions.Normal(action_mean, action_std)
        return distribution.sample()
    
        ####
        
        #raise NotImplementedError

    def update(self, observations, actions):
        
        #if isinstance(observations, np.ndarray):
        #    observations = torch.tensor(observations).float().to(ptu.device)
            
        #if isinstance(actions, np.ndarray):
        #    actions = torch.tensor(actions).float().to(ptu.device)
        """
        Updates/trains the policy

        :param observations: observation(s) to query the policy
        :param actions: actions we want the policy to imitate
        :return:
            dict: 'Training Loss': supervised learning loss
        """
        
        ## 
        action_mean = self.mean_net(torch.from_numpy(observations).float())
        action_std = torch.exp(self.logstd)
        distribution = distributions.Normal(action_mean, action_std)
        
    
        # TODO: update the policy and return the loss
        # 動作的均值和標準差來建立一個正態分布，然後使用這個分布來計算實際動作的負對數似然損失，這將考慮到策略的不確定性
        loss = -distribution.log_prob(torch.from_numpy(actions)).mean()
        
        self.optimizer.zero_grad() #Zero out the gradients
        loss.backward()
        self.optimizer.step() #Update the weights

        
        return {
            # You can add extra logging information here, but keep this line
            'Training Loss': ptu.to_numpy(loss),
        }
